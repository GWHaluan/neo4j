...
. +include+ - For a clearer structure I would have liked to split the content up a little.
. Custom IDs and references - I tried to add a custom ID to an image and then reference the image directly which did not work unfortunately.
